...
const editor = {
    ...
    replaceObject(str, skip = false, fix = false, autoGroup = false) {
        ...
        if (selected) {
            ...
            if (fix) {
                console.log(selected.rotation.y, THREE.Math.radToDeg(selected.rotation.y), selected.rotation.y < 0 ? -180 + THREE.Math.radToDeg(selected.rotation.y) : 360 - THREE.Math.radToDeg(selected.rotation.y));
                if (fix != 'RAMP') this.objConfigGUI.__controllers[1].setValue(false);
                if (['VEHICLE', 'CONTAINER', 'CONTAINERR', 'TREE'].includes(fix)) rotation = selected.rotation.y < 0 ? -180 + THREE.Math.radToDeg(selected.rotation.y) : 360 - THREE.Math.radToDeg(selected.rotation.y);
            }
